...
two socket patterns here:
  ...
  SUB (subscriber) — receives StatePackets broadcast by the LLI's PUB socket.
    The LLI sends one packet per 50 Hz tick; if the PC is slow it can fall behind.
...
class ZMQClient:
    ...
    Both sockets are blocking by default — recv_state() will wait up to ~20 ms
    for the next 50 Hz packet, which naturally paces the control loop.
    """
    ...
    def recv_state(self) -> StatePacket:
        ...
        Normally returns within ~20 ms (one 50 Hz tick). During homing the LLI
